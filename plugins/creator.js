...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode }) => {
 ...
 let ftroli = { key: { remoteJid: 'status@broadcast', participant: '0@s.whatsapp.net' }, message: { orderMessage: { itemCount: 9999999999, status: 1, surface: 1, message: `${command} ${type}`, orderTitle: wm, sellerJid: '0@s.whatsapp.net' } } }
  let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Ryzn⸙;;;\nFN:Ryzn⸙\nORG:Ryzn⸙\nTITLE:\nitem1.TEL;waid=6283822339970:+62 838-2233-9970\nitem1.X-ABLabel:Ryzn⸙\nX-WA-BIZ-DESCRIPTION:${htjava} Tidak menerima save kontak!!\nX-WA-BIZ-NAME:Ryzn⸙\nEND:VCARD`
//------- NOMOR
 const pp = fs.readFileSync('./media/ok.jpg')
...
let teksbio = `┆⫹⫺ 💌 Nama : Relz
┆⫹⫺ ✉️ Nama RL : Tebak aja
┆⫹⫺ ♂️ Gender : Pria
┆⫹⫺ 🕋 Agama : Islam
┆⫹⫺ ⏰ Tanggal lahir : Kepo nyett
┆⫹⫺ 🎨 Umur : 17
┆⫹⫺ 🧮 Kelas : -
┆⫹⫺ 🧩 Hobby : Ngoding
...
await conn.sendMessage(m.chat, { contacts: { displayName: wm, contacts: [{ vcard }] }}, { quoted: ftroli })
 ...
 conn.send2ButtonImg(m.chat, fs.readFileSync('./media/ok.jpg').buffer, teksnomor, `Halo ${tag}\nIᴛᴜ Oᴡɴᴇʀ Kᴜ⫺⫰⫹\n` + botdate, `Sewa Bot`, `.sewa`, 'Menu', `.menu`, ftroli, { contextInfo: { externalAdReply: { showAdAttribution: true,
   ...
   conn.sendButton(m.chat, '┍┈༺ *Biodata Ryzn* ༻', teksbio, ppown, [
                ['Sewa Bot', `${usedPrefix}sewa`],
                ['Menu', `${usedPrefix}menu`]
            ], m)
            break
